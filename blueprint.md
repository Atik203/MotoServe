...
- Node 24 · TypeScript strict · Inter font · brand blue `#0052cc` · npm (single package manager)
...
- **Owner verification docs**: registration "Identity Verification" has two multi-file dropzones (NID + License, JPG/PNG/PDF ≤5MB each) uploading via **S3 presigned PUT** (browser → S3 directly, no server round-trip); keys `MotoServe/docs/anon-{uuid}/…` pre-login (presign is auth-optional — logged-in uploads use `{userId}`); stored as `documents` JSON array `[{name, key, kind: nid|license}]` on User (legacy `documentUrl` kept for seed data). Profile photo uploads via `purpose: "image"` → `avatar` key (S3 keys rendered via presigned GET — any authenticated role, so owners resolve their own). Admin detail `/admin/verifications/[id]` fetches a 15-min **presigned GET** per document and renders images/PDFs inline. `GET /customers` selects safe fields (no passwordHash). Demo doc `backend/prisma/demo-nid.png` is uploaded to `MotoServe/demo/demo-nid.png` by the seed (best-effort, skips without AWS creds).
...
- Sections: **Personal Information** (profile photo upload tile, Full Name/Email/Phone/Password/Confirm (all required, red `*`), DOB date picker, Gender select, NID*, Driving License*, Occupation) → **Address Information** (Street*, City*, District/State\*, Zip, Country) → **Identity Verification** (two multi-file dropzones: NID + License docs).
