...
- Uploads: files go to **AWS S3** via presigned URLs — `POST /api/upload/presign` (5-min PUT, private keys under `MotoServe/{docs|images}/{userId}/…`) and `POST /api/upload/presign-get` (15-min GET, admin/advisor only). Env: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_BUCKET_NAME`, `AWS_REGION`, `S3_ROOT_PREFIX` (default `MotoServe/`). Bucket CORS must allow PUT from the client origin; bucket policy stays private (no public statements needed for MotoServe).
