...
- Node 24 LTS · npm (single package manager) · TypeScript strict everywhere · Inter font · primary brand blue `#0052cc` (canonical for ALL screens).
...
5. **Preserve pinned versions.** Do NOT run `npm update` or add packages at newer majors without asking. Verify a package exists with `npm view <pkg> version` before adding.
...
- shadcn components go in `src/components/ui/`; ONLY add with `npx shadcn@latest add <name>` (or copy the component when offline — keep props identical to current shadcn).
...
- Uploads: files go to **AWS S3** via presigned URLs — `POST /api/upload/presign` (5-min PUT, auth-optional: uses `{userId}` when logged in, `anon-{uuid}` pre-login; keys under `MotoServe/{docs|images}/{folderId}/…`) and `POST /api/upload/presign-get` (15-min GET, any authenticated role — owners resolve their own avatar/doc keys). Env: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_BUCKET_NAME`, `AWS_REGION`, `S3_ROOT_PREFIX` (default `MotoServe/`). Bucket CORS must allow PUT from the client origin; bucket policy stays private (no public statements needed for MotoServe).
- Payments: **Stripe Checkout** (card only) — `POST /api/payments/checkout` returns a hosted session URL; `POST /api/payments/webhook` (mounted with `express.raw()` BEFORE the JSON parser, signature-verified) marks the invoice PAID on `checkout.session.completed`. Mobile/cash stay dummy via `POST /invoices/:id/pay`. Env: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`. Local webhook dev: `stripe listen --forward-to http://localhost:4000/api/payments/webhook` (paste the printed `whsec_…` into `STRIPE_WEBHOOK_SECRET`). Test card `4242 4242 4242 4242`.
- Job lifecycle: completing a job (`PATCH /jobs/:id/status` → `completed`) auto-creates an UNPAID invoice (services + parts + estimate labor + 8.5% tax) via `ensureInvoiceForJob`. IDs are max-suffix based (`JC-/ES-/INV-`), never count-based.
- Chat: per-party unread counters (`ownerUnread`/`advisorUnread` on ChatThread — the API maps to `unread` per requester); `POST /chat/threads/:id/read` zeroes the caller's counter; sending increments the OTHER party's. Shared UI: `src/components/roles/chat/CommunicationCenter.tsx` (owner + advisor wrappers).
- Vehicles: owner-only `PATCH/DELETE /api/vehicles/:id` with ownership checks.
